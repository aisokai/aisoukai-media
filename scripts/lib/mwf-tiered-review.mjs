...
export function createTieredReviewer({ request, githubFile, githubDirectory, secret }) {
  return async function review({ raw, path, baselineRaw }) {
    try {
      ...
      const files=await githubDirectory('content/posts')
      if (!Array.isArray(files)||files.length>500) return {status:'draft',reason:'duplicate_scope_unavailable'}
      const comparisons=[]
      for(const file of files.filter(f=>f.type==='file'&&/^\d{4}-\d{2}-\d{2}-.+\.md$/.test(f.name)&&`content/posts/${f.name}`!==path)) {
        const other=matter(await githubFile(`content/posts/${file.name}`))
        if(isProtectedEditorialInput(other.data,other.content))return {status:'draft',reason:'protected_comparison'}
        if (normalizeStockTitle(other.data.title)===normalizeStockTitle(data.title) || (data.source_topic_id&&other.data.source_topic_id===data.source_topic_id)) return {status:'draft',reason:'duplicate_detected'}
        comparisons.push({title:other.data.title,excerpt:other.data.excerpt,category:other.data.category})
      }
